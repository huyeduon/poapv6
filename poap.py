#!/bin/env python3
# md5sum="88516b4fd347de58bb3655cf332268c4"
"""
    ...
    if unpack == True:
        ...
        if sp == None:
            ...
            try:
                ...
                poap_log("Failed to unpack %s: %s (%s)" %
                         (dst, str(e), e.output))
...
def install_shell_script(source_path, source_file):
    ...
    for i in range(0, 10):
        try:
            ...
            poap_log("Failed to chkconfig level 3 %s on: %s" %
                     (source_file, str(e)))
        else:
...
def set_defaults_and_validate_options():
    ...
    set_default("destination_kickstart_image",
    ...
    for file in rollback_files:
        ...
        if file.endswith(".rpm"):
            ...
            if (len(rpmgrp) != 0 and 'Patch-RPM' in rpmgrp):
                ...
                os.system(
                    "rm -rf /bootflash/.rpmstore/patching/patchrepo/%s" % file)
                os.system(
                    "rm -rf /bootflash_sup-remote/.rpmstore/patching/patchrepo/%s" % file)
                ...
                if (len(rpmtype) != 0 and 'feature' in rpmtype):
                    ...
                    os.system(
                        "rm -rf /bootflash_sup-remote/.rpmstore/patching/localrepo/%s" % file)
                    if (int(image_parts[0]) >= 10):
                        os.system(
                    ...
                    os.system(
                        "rm -rf /bootflash/.rpmstore/thirdparty/%s" % file)
                    os.system(
            ...
            rpm_name = subprocess.check_output(
                "/usr/bin/rpm -qp --qf %%{NAME} /bootflash/poap_files/%s" % file, shell=True)
            rpm_name = byte2str(rpm_name)
            rpm_persisted_removal_string = "sed -i '/^{0}$/d' /bootflash/.rpmstore/nxos_rpms_persisted" .format(
                rpm_name)
...
def mtc_shut_member_ports(line, config_file_first):
    ...
    if (port != 0):
        config_file_first.write('interface Ethernet' +
                                str(intf) + '/' + str(port) + '\n')
...
def split_config_file():
    ...
    if split_config_not_needed():
        ...
        line = ""
        poap_log("Skip split config as it isn't needed with %s" %
    ...
    if single_image == True:
        ...
        single_image_path = single_image_path.replace(
            "/bootflash", "bootflash:", 1)
    ...
    remove_file(os.path.join(
        options["destination_path"], options["destination_config"]))
    config_file_first.close()
    if empty_first_file == 1:
        remove_file(os.path.join(
            options["destination_path"], options["split_config_first"]))
...
def get_md5(filename, skip_abort=False):
    ...
    file_hdl = open(os.path.join(
        options["destination_path"], md5_filename), "r")
...
def do_copy(source="", dest="", login_timeout=10, dest_tmp="", compact=False, dont_abort=False):
    ...
    if os.environ.get("POAP_PHASE", None) == "USB":
        ...
        if os.path.exists(copy_src):
            ...
            abort("/usbslot%d/%s does NOT exist" %
                  (options["usb_slot"], source))
    else:
        ...
        if legacy:
            try:
                ...
                poap_log(
                    "Copy done using transfer module. Please check size below")
            except Exception as e:
                # Handle known cases
...
def copy_system():
    ...
    if options["disable_md5"] == False:
        copy_md5_info(options["target_image_path"],
                      options["target_system_image"])
    ...
    src = os.path.join(options["target_image_path"],
                       options["target_system_image"])
...
def install_images_7_x():
    ...
    system_image_path = os.path.join(options["destination_path"],
                                     options["destination_system_image"])
    system_image_path = system_image_path.replace(
        "/bootflash", "bootflash:", 1)
...
def install_nxos_issu():
    ...
    try:
        ...
        poap_log("terminal dont-ask ; install all nxos %s no-reload non-interruptive" %
                 system_image_path)
        cli("terminal dont-ask ; install all nxos %s no-reload non-interruptive" %
            system_image_path)
        time.sleep(5)
...
def parse_poap_yaml():
    ...
    """
    copy_path = options["install_path"] + "/" + \
        options["serial_number"] + "/" + options["serial_number"] + ".yaml"
    alt_path = options["install_path"] + "/" + \
        options["serial_number"] + "/" + options["serial_number"] + ".yml"
    timeout = options["timeout_copy_system"]
    ...
    try:
        if options["disable_md5"] == False:
            copy_md5_info(os.path.join(
                options["install_path"], options["serial_number"]), options["serial_number"] + ".yaml")
        ...
        try:
            ...
            if options["disable_md5"] == False:
                copy_md5_info(os.path.join(
                    options["install_path"], options["serial_number"]), options["serial_number"] + ".yml")
...
def copy_poap_files():
    ...
    if ("RPM" in dictionary):
        ...
        for rpm in dictionary["RPM"]:
            rpm = rpm.strip()
            name_str = "rpm -qp --qf '%{NAME}-%{VERSION}-%{RELEASE}.%{ARCH}.rpm' /bootflash/poap_files/" + rpm.split(
                '/')[-1]
            orig_name = subprocess.check_output(name_str, shell=True)
            orig_name = byte2str(orig_name)
            if (orig_name != rpm.split('/')[-1]):
                poap_log("ERROR : RPM file %s does not match RPM package naming convention. Expected name: %s" % (
...
def install_license():
    ...
    for file in os.listdir("/bootflash/poap_files"):
        if file.endswith(".lic"):
            ...
            tmp_file_write.write(
                "install license bootflash:poap_files/%s\n" % file)
...
def install_rpm():
    ...
    for file in os.listdir("/bootflash/poap_files"):
        if file.endswith(".rpm"):
            ...
            os.system('echo "' + file +
                      '" >> /bootflash/poap_files/success_install_list')
            group_string = "/usr/bin/rpm -qp --qf %{GROUP} /bootflash/poap_files/" + file
            rpm_string = "/usr/bin/rpm -qp --queryformat %{NXOSRPMTYPE} /bootflash/poap_files/" + file
            rpmgrp = subprocess.check_output(group_string, shell=True)
            rpmtype = subprocess.check_output(rpm_string, shell=True)
            if (len(rpmgrp) != 0 and 'Patch-RPM' in str(rpmgrp)):
                ...
                if (not check_if_rpm_in_file("/bootflash/.rpmstore/patching/patchrepo/meta/patching_meta.inf", patch_rpm_name)):
                    ...
                    os.system(
                        "cp /bootflash/poap_files/%s /bootflash/.rpmstore/patching/patchrepo/" % file)
                    os.system(
                        "cp /bootflash/poap_files/%s /bootflash_sup-remote/.rpmstore/patching/patchrepo/" % file)
                    if (int(image_parts[0]) >= 10):
                        os.system(
                            "sudo /usr/bin/createrepo_c --update /bootflash/.rpmstore/patching/patchrepo/")
                        os.system(
                ...
                if (len(rpmtype) != 0 and 'feature' in str(rpmtype)):
                    ...
                    os.system(
                        "cp /bootflash/poap_files/%s /bootflash/.rpmstore/patching/localrepo/" % file)
                    os.system(
                        "cp /bootflash/poap_files/%s /bootflash_sup-remote/.rpmstore/patching/localrepo/" % file)
                    if (int(image_parts[0]) >= 10):
                        os.system(
                            "sudo /usr/bin/createrepo_c --update /bootflash/.rpmstore/patching/localrepo/")
                        os.system(
                    ...
                    poap_log(
                        "RPM is a third-party RPM. Executing clis for the same")
                    os.system(
                        "cp /bootflash/poap_files/%s /bootflash/.rpmstore/thirdparty/" % file)
                    os.system(
                        "cp /bootflash/poap_files/%s /bootflash_sup-remote/.rpmstore/thirdparty/" % file)
                    if (int(image_parts[0]) >= 10):
                        os.system(
                            "sudo /usr/bin/createrepo_c --update /bootflash/.rpmstore/thirdparty/")
                        os.system(
                ...
                if not check_if_rpm_in_file("/bootflash/.rpmstore/nxos_rpms_persisted", rpm_name):
                    rpm_append_str = "/usr/bin/rpm -qp --qf %{NAME} /bootflash/poap_files/" + \
                        file + " >> /bootflash/.rpmstore/nxos_rpms_persisted"
                    os.system(rpm_append_str)
                    os.system(
                        'echo "" >> /bootflash/.rpmstore/nxos_rpms_persisted')
                    standby_append_str = "/usr/bin/rpm -qp --qf %{NAME} /bootflash/poap_files/" + \
                        file + " >> /bootflash_sup-remote/.rpmstore/nxos_rpms_persisted"
    ...
    if (patch_count > 0):
        if ((os.path.exists("/bootflash/.rpmstore/patching/patchrepo/meta/patching_meta.inf"))):
            ...
            if ("committed_list" in patching_meta):
                ...
                patch_append_str = 'echo "' + activate_list + \
                    '" >> /bootflash/.rpmstore/patching/patchrepo/meta/patching_meta.inf'
                standby_append_str = 'echo "' + activate_list + \
...
def install_certificate():
    ...
    stream = open("/bootflash/poap_device_recipe.yaml", 'r')
    dictionary = yaml.load(stream)
    config_file_second = open(os.path.join(
        "/bootflash", options["split_config_second"]), "a+")
...
def set_cfg_file_mac():
    ...
    if os.environ.get("POAP_PHASE", None) == "USB":
        ...
        if os.path.exists("/usbslot%d/%s" % (usbslot, config_file)):
            ...
            poap_log("Selected conf file name : %s" %
                     options["source_config_file"])
            return
...
def set_cfg_file_location():
    ...
    cdp_output = cli("show cdp neighbors interface %s" %
    ...
    cdp_info = [info for info in re.split(
        "\s+", " ".join(cdp_lines[i:])) if info != ""]
    ...
    options["source_config_file"] = options["source_config_file"].replace(
        "/", "_")
    poap_log("Selected conf file name : %s" % options["source_config_file"])
    ...
    if legacy:
        ...
        if result != None and option != 1:
            ...
            if 'build' in interim_result:
                ...
                final_version = interim_result.replace(
                    '(', '.').replace(')', '')
    ...
    if re.match("nxos.", options["target_system_image"]):
        ...
        try:
            ...
            try:
                ...
                poap_log(
                    "Could not convert BIOS '%s' to a number, using text match")
                bios_number = bios
        try:
        ...
        poap_log("Comparing present BIOS version %d with base version %d" %
...
def set_next_upgrade_from_upgrade_path():
    ...
    if image_info == None:
        ...
        if image_info == None:
            poap_log("Failed to match target image: %s" %
                     options["target_system_image"])
            exit(1)
    ...
    if (target_idx - current_idx) > 0:
        ...
        poap_log("Next upgrade is %s from %s" %
...
def download_personality_tarball():
    ...
    md5_sum_given = None
    if options["disable_md5"] == False:
        copy_md5_info(options["personality_path"],
    ...
    tarball_path = os.path.join(
        options["personality_path"], options["source_tarball"])
...
def override_options_for_personality():
    ...
    poap_log("target_image_path option set to personality_path (%s)" %
...
def setup_mode():
    ...
    if options["mode"] == "location":
        ...
        poap_log("Mode must be one of the following: %s" %
...
def main():
    ...
    if empty_first_file == 0:
        cli('copy bootflash:%s scheduled-config' %
            options["split_config_first"])
